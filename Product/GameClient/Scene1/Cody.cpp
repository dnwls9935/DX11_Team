...
Cody::Cody(bool bOperationPad) noexcept
	: _mayTarget(nullptr)
{
...
void Cody::Start(void) noexcept
{
	...
	_renderer->SetModel(ASSETMANAGER->GetModelData("Asset\\Models\\Cody\\Cody_FSM_01.fbx"));
	...
	_animator->Play("Cody.ao|Cody_Mh", true);
	...
	_leftHandBone = gameObject->GetComponent<ModelRenderer>()->GetModel()->GetHierarchyNode("LeftHand_IK");
	_PickUpBone = gameObject->GetComponent<ModelRenderer>()->GetModel()->GetHierarchyNode("Align");

	/* Camera Bone */
	GameObject* cameraBone = GameObject::Instantiate("CodyCamera");
	...
	/* ============ MovementAudio ==============*/
	_audioasset[static_cast<uint>(eAudioType::CROUCH)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Footstep_Default_Crouch_Hard_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::WALK)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Characters_Movement_Cody_Idle_Footstep.ogg");
	_audioasset[static_cast<uint>(eAudioType::LAND)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Characters_Movement_Cody_Land_Default_Heavy.ogg");
	_audioasset[static_cast<uint>(eAudioType::SLIDESTOP)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Slide_Ass_Default_Stop_Smooth.ogg");
	_audioasset[static_cast<uint>(eAudioType::SLIDESTART)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Slide_Ass_Default_Start_Smooth.ogg");
	_audioasset[static_cast<uint>(eAudioType::SLIDELOOP)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Slide_Ass_Default_Loop_Smooth.ogg");
	_audioasset[static_cast<uint>(eAudioType::JUMP)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Jump_Default.ogg");
	_audioasset[static_cast<uint>(eAudioType::GROUNDPOUNDFALL)]		= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Action_GroundPound_Default_Whoosh.ogg");
	_audioasset[static_cast<uint>(eAudioType::GROUNDPOUNDSTART)]	= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Action_GroundPound_Default_InAir_Movement.ogg");
	_audioasset[static_cast<uint>(eAudioType::GROUNDPOUNDLAND)]		= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Action_GroundPound_Default_Impact.ogg");
	_audioasset[static_cast<uint>(eAudioType::DOUBLEJUMP)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Action_DoubleJump.ogg");
	_audioasset[static_cast<uint>(eAudioType::DASH)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_Character_Movement_Cody_Action_Dash.ogg");
								
	/* ================= Voice =================*/
	_audioasset[static_cast<uint>(eAudioType::VO_DASH)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Dash_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_AIRDASH)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Air_Dash_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_DOUBLEJUMP)]		= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_DoubleJump_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_GROUNDPOUNTSTART)] = ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Ground_Pound_Start_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_GROUNDPOUNDJUMP)]	= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Ground_Pound_Jump_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_GROUNDPOUNDLAND)]	= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Ground_Pound_Land_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_JUMP)]				= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Jump_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_WALLGRAB)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Wall_Grab_Cody.ogg");
	_audioasset[static_cast<uint>(eAudioType::VO_WALLJUMP)]			= ASSETMANAGER->GetAudioAsset("Asset\\Audio\\Cody\\Play_VO_Gameplay_Efforts_Wall_Jump_Away_Cody.ogg");
	...
	{
		_leftFootDustFX = GameObject::Instantiate("codyLeftFootDustFX");
		DustGenerate* DustFX = new DustGenerate(transform);
		_leftFootDustFX->AddComponent(DustFX);

		_rightFootDustFX = GameObject::Instantiate("codyRightFootDustFX");
		DustFX = new DustGenerate(transform);
		_rightFootDustFX->AddComponent(DustFX);
	}

	_wareHouseNarrative = false;
}

void Cody::FixedUpdate(float fElapsedTime) noexcept
{
	...
	if (true == _isInteraction && nullptr != _interactObj)
	{
		...
		else if ("DumbbellCodyGrab" == _interactObj->GetName())
		{
			uint8 dirInput = static_cast<PlayerInput*>(_inputHandler)->GetDirInput();
			if (dirInput == 1 << 0)
			{
	...
	if (_isDead && !_isDeadEnd)
	{
		...
		{
			transform->SetWorldPosition(PlayerPrefs::This().GetFloat("SavePointX") -1.f ,
	...
	{
		if ((0.f <= _animator->GetCurrentProgress() && 0.1f > _animator->GetCurrentProgress())||
			(0.6f <= _animator->GetCurrentProgress() && 0.7f > _animator->GetCurrentProgress()))
			_leftFootDustFX->GetComponent<DustGenerate>()->SetPlay(true);
		if ((0.3f <= _animator->GetCurrentProgress() && 0.4f > _animator->GetCurrentProgress()) ||
			(0.8f <= _animator->GetCurrentProgress() && 0.9f > _animator->GetCurrentProgress()))
			_rightFootDustFX->GetComponent<DustGenerate>()->SetPlay(true);
	}

}

void Cody::Update(float fElapsedTime) noexcept
{
	if (OptionPopup::This()->IsShow())
		return;
...
void Cody::LateUpdate(float fElapsedTime) noexcept
{
	if (OptionPopup::This()->IsShow())
		return;
	...
	if (_isDirtyState)
	{
		...
		case Actor::State::IDLE:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Mh"
					, true);
			}
			else
			{
		...
		case Actor::State::WALKSTART:
			_animator->Play("Cody.ao|Cody_Loco_Jog_Start_Fwd"
				, false
				, std::bind(&Cody::OnEndWalk, this));
			_animator->SetPlaySpeed(1.f);
			_rigidbody->SetAngularDrag(1.f);
			_rigidbody->SetDrag(1.f);
			break;
		case Actor::State::JUMPLANDWALKSTART:
			_animator->Play("Cody.ao|Cody_Loco_Jog_ActionMh_Start_InMotion"
				, false
				, std::bind(&Cody::OnEndWalk, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::WALK:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Loco_Jog"
					, true);
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Loco_Jog"
					, true);
			}
			_animator->SetPlaySpeed(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::WALKEND:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Loco_Jog_Stop"
					, false
					, std::bind(&Cody::OnEndWalk, this));
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Loco_Jog_Stop_Fwd"
					, false
					, std::bind(&Cody::OnEndWalk, this));
			}
			...
			_movementEffectPlayer->SetVolume(1.f);
			_movementEffectPlayer->Stop();

			break;
		case Actor::State::RUN:
			_animator->Play("Cody.ao|Cody_Loco_Sprint"
				, true
				, std::bind(&Cody::OnEndWalk, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::DASH:
			...
			if (true == _OnGround)
			{
				...
				_animator->Play("Cody.ao|Cody_Trav_Roll_Start"
					, false
					, std::bind(&Cody::OnEndDash, this)
					, 0.9f
					, std::bind(&Cody::OnTriggerDash, this));

				_voiceEffectPlayer->Play(_audioasset[static_cast<UINT>(eAudioType::VO_DASH)],1.f,false);
			}
			else
			{
				...
				_animator->Play("Cody.ao|Cody_Trav_AirDash_Start"
					, false
					, std::bind(&Cody::OnEndDash, this)
					, 0.4f
					, std::bind(&Cody::OnTriggerDash, this));

				_voiceEffectPlayer->Play(_audioasset[static_cast<UINT>(eAudioType::VO_AIRDASH)],1.f,false);
			}
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::DASHEND:
			_animator->SetPlaySpeed(1.f);
			_animator->Play("Cody.ao|Cody_Trav_Roll_Stop"
				, false
				, std::bind(&Cody::OnEndDashEnd, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::DASHENDJOG:
			_animator->SetPlaySpeed(1.f);
			_animator->Play("Cody.ao|Cody_Trav_Roll_To_Jog"
				, false
				, std::bind(&Cody::OnEndDashEnd, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::JUMPFWD:
			...
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Trav_Jump_Start"
					, false
					, std::bind(&Cody::OnEndJump, this));
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_Jump_Start"
					, false
					, std::bind(&Cody::OnEndJump, this));
			}
			_rigidbody->SetDrag(1.f);
			...
			_movementEffectPlayer->Play(_audioasset[static_cast<UINT>(eAudioType::JUMP)],0.0025f,false);

			break;
		case Actor::State::JUMPLAND:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Trav_Jump_Land"
					, false
					, std::bind(&Cody::OnEndLand, this));
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_Jump_Land"
					, false
					, std::bind(&Cody::OnEndLand, this));
			}
			_rigidbody->SetDrag(1.f);
		...
		case Actor::State::JUMPLANDJOG:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Trav_Jump_Land_Jog"
					, false
					, std::bind(&Cody::OnEndLand, this));
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_Jump_Land_Jog"
					, false
					, std::bind(&Cody::OnEndLand, this));
			}
			_rigidbody->SetDrag(1.f);
		...
		case Actor::State::SECOND_JUMP:
			...
			_animator->Play("Cody.ao|Cody_Trav_DoubleJump"
				, false
				, std::bind(&Cody::OnEndJump, this));
		...
		case Actor::State::FALLING:
			if (_isPickUp)
			{
				_animator->Play("Cody.ao|Cody_Bhv_PickUp_Trav_Jump_Falling"
					, true);
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_Jump_Falling"
					, true);
			}
		...
		case Actor::State::CROUCHENTER:
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Enter_Mh"
				, false
				, std::bind(&Cody::OnEndCrouch, this));
			break;
		case Actor::State::CROUCHSTART:
			_animator->SetPlaySpeed(2.f);
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Start"
				, false
				, std::bind(&Cody::OnEndCrouch, this));
			break;
		case Actor::State::CROUCHIDLE:
			_animator->SetPlaySpeed(1.f);
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Mh"
				, true);
			break;
		case Actor::State::CROUCH:
			_animator->SetPlaySpeed(1.f);
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Fwd"
				, true);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);

			_movementEffectPlayer->Play(_audioasset[static_cast<UINT>(eAudioType::CROUCH)],0.005f);

			break;
		case Actor::State::CROUCHEND:
			_animator->SetPlaySpeed(2.f);
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Stop"
				, false
				, std::bind(&Cody::OnEndCrouch, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::CROUCHEXIT:
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Exit_Mh"
				, false
				, std::bind(&Cody::OnEndCrouch, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::CROUCHEXITMOVE:
			_animator->Play("Cody.ao|Cody_Trav_Crouch_Exit_Movement"
				, false
				, std::bind(&Cody::OnEndCrouch, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::GROUNDPOUNDSTART:
			_rigidbody->SetUseGravity(false);
			_animator->SetPlaySpeed(1.5f);
			_animator->Play("Cody.ao|Cody_Bhv_GroundPound_Start"
				, false
				, std::bind(&Cody::OnEndGroundPound, this));
		...
		case Actor::State::GROUNDPOUNDFALLING:
			...
			_animator->Play("Cody.ao|Cody_Bhv_GroundPound_Falling"
				, true);
		...
		case Actor::State::GROUNDPOUNDLAND:
			...
			PADINPUT->Vibration(1.f, 1.f, 0.35f, ce::CETween::EaseType::easeInBack);
			_animator->Play("Cody.ao|Cody_Bhv_GroundPound_Land"
				, false
				, std::bind(&Cody::OnEndGroundPound, this));
		...
		case Actor::State::GROUNDPOUNDEXIT:
			_animator->SetPlaySpeed(2.f);
			_animator->Play("Cody.ao|Cody_Bhv_GroundPound_Land_Exit"
				, false
				, std::bind(&Cody::OnEndGroundPound, this));
		...
		case Actor::State::PICKUP:
			_animator->Play("Cody.ao|Cody_Bhv_PickUp"
				, false
				, std::bind(&Cody::OnEndPickUp, this));
			_animator->SetPlaySpeed(1.f);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::PICKENTER:
			_animator->Play("Cody.ao|Cody_Bhv_PickUp_FuseCrab_PutInSocket"
				, false
				, std::bind(&Cody::OnEndPickUp, this)
				, 0.7f
		...
		case Actor::State::WALLSLIDERENTER:
			...
			if (_isDash)
			{
				_animator->Play("Cody.ao|Cody_Trav_WallSlide_Enter_FromDash"
					, false
					, std::bind(&Cody::OnEndWallSlide, this)
					, 0.f
					, std::bind(&Cody::WallJump, this));
			}
			/* 점프 , 이단점프 일때*/
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_WallSlide_Enter"
					, false
					, std::bind(&Cody::OnEndWallSlide, this)
					, 0.f
					, std::bind(&Cody::WallJump, this));
			...
			if ("MachineGear" == _interactObj->GetName() ||
				"WallSlider" == _interactObj->GetName())
			{
		...
		case Actor::State::WALLSLIDEIDLE:
			_animator->Play("Cody.ao|Cody_Trav_WallSlide_MH"
				, true);
		...
		case Actor::State::WALLSLIDEJUMP:
			if (dirInput & 1 << 0)
			{
				_animator->Play("Cody.ao|Cody_Trav_WallSlide_Jump_Vertical"
					, false
					, std::bind(&Cody::OnEndWallSlide, this));
			}
			else
			{
				_animator->Play("Cody.ao|Cody_Trav_WallSlide_Jump"
					, false
					, std::bind(&Cody::OnEndWallSlide, this));
			}
			_rigidbody->SetDrag(1.f);
			...
			XMVECTOR vecWallJumpdir = XMVector3Normalize(XMVectorSet( -XMVectorGetX(transform->GetForward()), 1, -XMVectorGetZ(transform->GetForward()), 0));
		...
		case Actor::State::SAWENTER:
			_animator->Play("Cody.ao|Cody_EV_Shed_Awakening_Saw_Enter"
				, false
				, std::bind(&Cody::OnEndSawJump, this));
			_animator->SetPlaySpeed(0.f);
			break;
		case Actor::State::SAWIDLE:
			_animator->Play("Cody.ao|Cody_EV_Shed_Awakening_Saw_Mh"
				, true);
			_animator->SetPlaySpeed(1.f);
			break;
		case Actor::State::SAWSUCCSESS:
			_animator->Play("Cody.ao|Cody_EV_Shed_Awakening_Saw_Succsess"
				, false
				, nullptr
				, 0.79f
				, std::bind(&Cody::OnEndSawJump, this));
		...
		case Actor::State::SLIDEENTER:
			_animator->Play("Cody.ao|Cody_Trav_Slide_Enter"
				, false
				, std::bind(&Cody::OnEndSlide, this));
		...
		case Actor::State::SLIDEIDLE:
			_animator->Play("Cody.ao|Cody_Trav_Slide_Fwd"
				, true);
			_rigidbody->SetDrag(0);
		...
		case Actor::State::SLIDEEXIT:
			_animator->Play("Cody.ao|Cody_Trav_Slide_Exit_Falling"
				, false
				, std::bind(&Cody::OnEndSlide, this));
		...
		case Actor::State::VALVE_ROTATE_IDLE:
			_animator->Play("Cody.ao|Cody_Bhv_Generic_Valve_Rotate_Mh"
							, true);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::VALVE_ROTATE:
			_animator->Play("Cody.ao|Cody_Bhv_Generic_Valve_Rotate_R_Short"
							, true);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::PUSHENTER:
			_animator->Play("Cody.ao|Cody_Bhv_Push_Enter"
							, false
							, std::bind(&Cody::OnEndPush, this));
			_rigidbody->SetFreezeRotationY(true);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::PUSHIDLE:
			_animator->Play("Cody.ao|Cody_Bhv_Push_MH"
							, true);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			break;
		case Actor::State::PUSH:
			_animator->Play("Cody.ao|Cody_Bhv_Push_Fwd"
							, true
							, std::bind(&Cody::OnEndPush, this));
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
		...
		case Actor::State::PUSHEND:
			_animator->Play("Cody.ao|Cody_Bhv_Push_Exit"
							, false
							, std::bind(&Cody::OnEndPush, this));
		...
		case Actor::State::LEVERLEFT:
			_animator->Play("Cody.ao|Cody_Bhv_Lever_Left"
				, false
				, std::bind(&Cody::OnEndLever, this)
				, 0.3f
				, std::bind(&Cody::InteractLeverReverse, this)
			);
			_rigidbody->SetDrag(1.f);
			_rigidbody->SetAngularDrag(1.f);
			_animator->SetPlaySpeed(0.f);
			...
			break;
		case Actor::State::LEVERRIGHT:
			_animator->Play("Cody.ao|Cody_Bhv_Lever_Right"
				, false
				, std::bind(&Cody::OnEndLever, this)
				, 0.3f
				, std::bind(&Cody::InteractLeverReverse, this)
...
void Cody::Idle(void) noexcept
{
	...
	default:
		...
		{
			GameObject::FindObjectByName("Cinematic0")->GetComponent<WareHouseCinematic>()->SetOn(1);
		}
		_dirtyState = State::IDLE;
		_isDirtyState = true;
...
void Cody::Die(void) noexcept
{
}

void Cody::Dash(void) noexcept
{
	if (_state == State::ATTACK
		|| _state == State::DASH
		|| _state == State::SAWIDLE
...
void Cody::DoubleJump(void) noexcept
{
}

void Cody::Falling(void) noexcept
{
	if (_state == State::ATTACK ||
		_state == State::SUCTION ||
...
void Cody::SetDirection(float deltatime) noexcept
{
...
void Cody::CalcDirection(float angle, float deltatime) noexcept
{
	if (_state == Actor::State::ATTACK)
		return;
...
void Cody::Interaction(void) noexcept
{
	...
	{
		if (!_wareHouseNarrative && _interactObj->GetName() != "FuseCrab2")
		{
			if (!NarrationManager::This().SetCsvData("Asset\\Narrative\\Ware_House\\Ware_House_01_JumpOut.csv"))
			{
				CE_ASSERT("rldnr", "CSV DATA가 없습니다.");
			}
			NarrationManager::This().Play();
			_wareHouseNarrative = true;
		}
	...
	else if (_interactObj->GetName() == "DumbbellCodyGrab")
	{
		switch (_state)
		{
		case State::IDLE:
...
void Cody::Crouch(void) noexcept
{
	switch (_state)
	{
	case State::IDLE:
...
void Cody::Suction(float fElapsedTime) noexcept
{
	if (nullptr == _interactObj)
		return;
	...
	/*	if (true == ("VacuumFan0" != _interactObj->GetName()) &&
		...
		if ("SuctionEffect1_Enter" == _interactObj->GetName())
		{
				...
				GameObject* cam = GameObject::FindObjectByName("Cody_Cam");
				...
				cam->GetComponent<PathFollower>()->Play(1.5f, false, [&]() {
					auto cody = GameObject::FindObjectByName("Cody");
					cody->SetActive(true);
					auto next = GameObject::FindObjectByName(_interactObj->GetComponent<SuctionEffect>()->GetNext());
					cody->GetTransform()->SetWorldPosition(next->GetTransform()->GetWorldPosition());
		...
		else if ("SuctionEffect2_Enter" == _interactObj->GetName())
		{
				...
				cam->GetComponent<PathFollower>()->Play(1.f, false, [&]() {
					...
					cody->GetComponent<Rigidbody>()->AddForce(forwardwithoutY);
...
void Cody::WallJump(void) noexcept
{
	if (_isWallSlide)
	{
		transform->Rotate(0, XMConvertToRadians(180.f), 0);
...
void Cody::Attack(void) noexcept
{
}

void Cody::GetHit(float damage, int8 attacker) noexcept
{
}

void Cody::OnEndJump(void) noexcept
{
	_animator->SetPlaySpeed(1.f);
	_isDirtyState = true;
...
void Cody::OnEndLand(void) noexcept
{
	switch (_state)
	{
	case State::JUMPLAND:
...
void Cody::OnEndDash(void) noexcept
{
	_isDash = false;
	if (_OnGround)
	{
...
void Cody::OnEndDashEnd(void) noexcept
{
	switch (_state)
	{
	case State::DASHEND:
...
void Cody::OnEndWalk(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndCrouch(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndGroundPound(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndPickUp(void) noexcept
{
	switch (_state)
	{
	case State::PICKUP:
...
void Cody::OnEndWallSlide(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndSawJump(void) noexcept
{
	switch (_state)
	{
	case Actor::State::SAWENTER:
		...
		_interactObj->GetComponent<CircularSaw>()->SetAwake(CircularSaw::PLAYER::CODY, true);
...
void Cody::OnEndSlide(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndPush(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndInteraction(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEndLever(void) noexcept
{
	switch (_state)
	{
...
void Cody::OnEnterInteraction(void) noexcept
{
	switch (_state)
	{
	...
	if (nullptr != _interactObj)
	{
		...
		{
			_interactObj->GetComponent<PowerSwitch>()->SetLower(1, true);
		}
	}
}

void Cody::UpdateCheckEvent(float fElapsedTime) noexcept
{
	...
	else if (objectName == "DumbbellCodyGrab")
...
void Cody::LateUpdateInteractStart(const std::string& objectName) noexcept
{
	if (objectName == "PowerSwitch")
	{
		_animator->Play("Cody.ao|Cody_Trav_Jump_Start"
			...
			, nullptr
			, 0.3f
			, std::bind(&Cody::OnEnterInteraction, this));
	}
}

void Cody::LateUpdateInteract(const std::string& objectName) noexcept
{
	if (objectName == "PowerSwitch")
	{
		_animator->Play("Cody.ao|Cody_Bhv_Shed_FuseLever_Enter"
			, false
			, std::bind(&Cody::OnEnterInteraction, this));
	}
	else if (objectName == "FuseCrab")
	{
		_animator->Play("Cody.ao|Cody_Bhv_PickUp_FuseCrab_PutInSocket"
			, false
			, std::bind(&Cody::OnEndInteraction, this));
	}
}

void Cody::LateUpdateInteractIdle(const std::string& objectName) noexcept
{
	if (objectName == "PowerSwitch")
	{
		_animator->Play("Cody.ao|Cody_Bhv_Shed_FuseLever_MH"
			, true);
	}
}

void Cody::LateUpdateInteractEnd(const std::string& objectName) noexcept
{
	if (objectName == "PowerSwitch")
	{
		_animator->Play("Cody.ao|Cody_Bhv_Shed_FuseLever_Success"
			, false
			, std::bind(&Cody::OnEndInteraction, this));
		_interactObj = nullptr;
	}
...
void Cody::OnTriggerDash(void) noexcept
{
	if (true == _OnGround)
	{
		if (!static_cast<PlayerInput*>(_inputHandler)->GetDirInput())
...
void Cody::OnTriggerEnter(Collider* mine, Collider* other) noexcept
{
...
void Cody::OnTriggerExit(Collider* mine, Collider* other) noexcept
{
	GameObject* obj = other->GetGameObject();
	switch (obj->GetTag())
	{
...
void Cody::InteractPowerSwitch(float fElapsedTime) noexcept
{
	if (_interactObj == nullptr)
		return;
		...
		|| _state == Actor::State::INTERACTIONIDLE)
	{
		...
		{
			XMMATRIX CodyGrabMatrix = _interactObj->GetComponent<PowerSwitch>()->GetCodyGrabMatrix();

			XMMATRIX rightHandBoneMatrix = _leftHandBone->GetCombinedTransform()
			...
			XMVECTOR pos = XMVectorSet(XMVectorGetX(CodyGrabMatrix.r[3]) - 0.1f
				, XMVectorGetY(CodyGrabMatrix.r[3]) - dist + 0.35f
				, XMVectorGetZ(CodyGrabMatrix.r[3]) - 0.1f
				, 1.f);
			...
			if (_state == Actor::State::INTERACTIONIDLE)
			{
				...
				if (40.f <= _interactObj->GetComponent<PowerSwitch>()->GetCurrentAngle())
				{
					_isInteraction = false;
					_interactime = 0.f;
					_isDirtyState = true;
...
void Cody::InteractFuseCrab(float fElapsedTime) noexcept
{
	if (_interactObj == nullptr)
		return;
...
void Cody::InteractSaw(float fElapsedTime) noexcept
{
	if (_interactObj == nullptr)
		return;
	...
	if (_state == Actor::State::SAWENTER)
	{
		...
		XMMATRIX codyGrabMatrix = _interactObj->GetComponent<CircularSaw>()->GetGrabMatrix(CircularSaw::PLAYER::CODY);

		XMVECTOR pos = XMVectorSet(XMVectorGetX(codyGrabMatrix.r[3])
			, XMVectorGetY(codyGrabMatrix.r[3])
			, XMVectorGetZ(codyGrabMatrix.r[3]) - 0.35f
			, 1.f);
...
void Cody::InteractPumpValve(float fElapsedTime) noexcept
{
	if (nullptr == _interactObj)
		return;
...
void Cody::InteractPushDumbbell(float fElapsedTime) noexcept
{
	if (nullptr == _interactObj)
		return;
...
void Cody::InteractLever(float fElapsedTime) noexcept
{
	...
	if (!_isInteraction)
	{
		...
		float fx = CETween::Lerp(_lerpPosition.x, XMVectorGetX(_interactObj->GetTransform()->GetWorldPosition()), std::min(1.f, _interactime), CETween::EaseType::easeOutCubic);
		float fz = CETween::Lerp(_lerpPosition.z, XMVectorGetZ(_interactObj->GetTransform()->GetWorldPosition()), std::min(1.f,_interactime ), CETween::EaseType::easeOutCubic);
...
void Cody::InteractLeverReverse(void) noexcept
{
	if (_interactObj->GetName() == "LeverPosition")
	{
...
void Cody::InteractMachineGear(float fElapsedTime) noexcept
{
...
void Cody::InteractWallslider(float fElapsedTime) noexcept
{
}
